...
const getAllDoc = asyncHandler(async (req, res): Promise<any> => {
  const { _id } = req.user;
  if (!_id) {
...
const createNewDoc = asyncHandler(async (req, res): Promise<any> => {
  const { _id } = req.user;
...
const deleteDocWithId = asyncHandler(async (req, res): Promise<any> => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(true, 200, "Document delete successfully", deleteDoc)
    );
});
