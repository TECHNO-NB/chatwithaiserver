...
import { ISignupInput } from "../types/types.js";
...
const signupController = asyncHandler(async (req, res): Promise<any> => {
  const { email, password }: ISignupInput = req?.body;
...
const verifyEmail = asyncHandler(async (req, res): Promise<any> => {
  ...
  const user = await User.findOne({ verifiedCode: parseInt(code) }).select(
    "-password"
  );
  ...
  return (
    res
      ...
      .cookie("refreshToken", refreshToken, options)
      .json(
        new ApiResponse(true, 200, "Email verified successfully", verifiedUser)
      )
  );
});

const loginWithGoogle = asyncHandler(async (req, res): Promise<any> => {
  ...
  if (existingUser) {
    ...
    const findUser = await User.findOne({ email: token.email }).select(
      "-password"
    );
    if (!findUser) {
...
const verfiyUser = asyncHandler(async (req, res): Promise<any> => {
  const { _id } = req.user;
  if (!_id) {
...
const upgradePlan = asyncHandler(async (req, res): Promise<any> => {
  const { id } = req.params;
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(true, 200, "User upgrade plan successfully", findUser)
    );
});
export {
  signupController,
